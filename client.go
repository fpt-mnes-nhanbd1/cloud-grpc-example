...
func getBook(ctx context.Context, args ...string) {
	if len(args) != 1 {
	...
	defer conn.Close()
	r, err := client.Get(ctx, &pb.BookIdRequest{
		Id: int32(id),
	})
	if err != nil {
...
func deleteBook(ctx context.Context, args ...string) {
	if len(args) != 1 {
	...
	conn, client := GetClient()
	defer conn.Close()
	r, err := client.Delete(ctx, &pb.BookIdRequest{
		Id: int32(id),
	})
	if err != nil {
...
func listBooks(ctx context.Context, args ...string) {
	conn, client := GetClient()
	defer conn.Close()
...
func addBook(ctx context.Context, args ...string) {
	if len(args) != 3 {
...
func streamBook(ctx context.Context, args ...string) {
	conn, client := GetClient()
	defer conn.Close()
